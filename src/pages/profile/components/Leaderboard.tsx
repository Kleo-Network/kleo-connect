...
const Leaderboard = ({
  ...
  const { data: topUsersData, fetchData: fetchTopUsers } = useFetch()
  ...
  useEffect(() => {
    if (topUsersData) {
      ...
      const topUserIndex =
        (topUsersData as ILeaderboardData[])[0].address === userAddress ? 1 : 0
      setHighestKleoPoints(
        (topUsersData as ILeaderboardData[])[topUserIndex].kleo_points
      )
    }
    setIsLoading(false)
  }, [topUsersData])
